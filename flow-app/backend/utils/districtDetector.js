const DISTRICTS = [
  ...
  { name: 'Electronic City', bounds: { minLat:12.830, maxLat:12.880, minLng:77.640, maxLng:77.710 } },
...
function getDistrictForPoint(lat, lng) {
  for (const d of DISTRICTS) {
    if (lat >= d.bounds.minLat && lat <= d.bounds.maxLat &&
...
function getDistrictsFromRoute(route) {
  ...
  for (let i = 1; i < route.length; i++) {
    ...
    const prev = Array.isArray(route[i-1])
      ? { lat: route[i-1][0], lng: route[i-1][1] }
      : route[i-1];
    const curr = Array.isArray(route[i])
      ? { lat: route[i][0], lng: route[i][1] }
      : route[i];

    const district = getDistrictForPoint(curr.lat, curr.lng);
