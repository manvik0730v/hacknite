...
import { FiSearch, FiUserPlus, FiCheck, FiX, FiChevronRight, FiZap } from 'react-icons/fi';
      ...
      <div className={`rounded-3xl p-6 w-full max-w-sm ${sinMode?'bg-[#140000] border border-red-900':'bg-white'}`}
        ...
        <div className="flex flex-col items-center mb-5">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="flex items-center gap-0.5 text-xs text-[var(--text2)]">
              <FiZap size={10} className="text-yellow-500" />{profile.xp || 0} XP
            </span>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-5">
          {[
            ...
            { label:'Districts',   value: profile.districtsDon || 0, icon: <GiCrown size={12} className="text-yellow-500 mr-1 inline" /> },
            ...
            <div key={label} className={`rounded-xl p-3 text-center ${sinMode?'bg-red-950':'bg-blue-50'}`}>
              ...
              <p className="font-black text-[var(--text)]">{icon}{value}</p>
            </div>
          ))}
        </div>
