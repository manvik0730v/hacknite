...
export default function OnboardingPage() {
  ...
  const inputClass = "w-full bg-[var(--bg3)] text-[var(--text)] border border-[var(--border)] rounded-xl p-3 outline-none focus:border-[var(--accent)] transition";
      ...
      <div className="w-full max-w-sm">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-4">
            <h2 className="text-2xl font-bold text-center text-[var(--text)]">What should we call you?</h2>
            ...
            <input className={inputClass} placeholder="Username"
              ...
              onKeyDown={e => e.key === 'Enter' && data.username.trim() && setStep(2)} />
            ...
            <button onClick={skipAll}
              className="text-[var(--text2)] text-sm text-center py-2 hover:text-[var(--text)] transition">
              Skip all setup →
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="flex flex-col gap-4">
            <h2 className="text-2xl font-bold text-center text-[var(--text)]">Your body stats</h2>
            ...
            <button onClick={() => setStep(3)}
              className="w-full bg-[var(--accent)] text-white font-bold py-3 rounded-xl transition">
              Continue
            </button>
            <button onClick={skipAll}
              className="text-[var(--text2)] text-sm text-center py-2 hover:text-[var(--text)] transition">
        ...
        {step === 3 && (
          <div className="flex flex-col gap-4 items-center">
            <h2 className="text-2xl font-bold text-center text-[var(--text)]">Profile picture</h2>
            <p className="text-[var(--text2)] text-center text-sm">Optional — you can change this later</p>

            {/* Avatar + upload button */}
            <div className="relative">
              <div className="w-28 h-28 rounded-full bg-[var(--bg3)] border-2 border-[var(--accent)] flex items-center justify-center overflow-hidden">
                {data.profilePhoto
                  ? <img src={data.profilePhoto} className="w-full h-full object-cover" alt="avatar" />
              ...
              <button onClick={() => fileInputRef.current.click()}
                className="absolute bottom-0 right-0 w-9 h-9 rounded-full bg-[var(--accent)] text-white text-xl font-bold flex items-center justify-center shadow-lg border-2 border-[var(--bg)] hover:scale-110 transition">
                +
              </button>
            ...
            {data.profilePhoto && (
              <button onClick={() => update('profilePhoto', '')}
